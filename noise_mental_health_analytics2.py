...
# Complete Production-Ready Implementation - FIXED VERSION
...
class MentalHealthDataProcessor:
    ...
    def _create_composite_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if normalized_cols:
            ...
            # IMPROVED: Create composite from available health indicators
            health_indicators = []
            if 'sleep_quality' in df.columns:
                # Invert sleep quality (higher sleep quality = lower stress)
                health_indicators.append(10 - df['sleep_quality'])
            if 'anxiety_level' in df.columns:
                health_indicators.append(df['anxiety_level'])
            if 'mood_rating' in df.columns:
                # Invert mood rating (higher mood = lower stress)
                health_indicators.append(10 - df['mood_rating'])
            if 'depression_score' in df.columns:
                health_indicators.append(df['depression_score'])
            if 'cortisol_level' in df.columns:
                # Normalize cortisol level to 0-10 scale
                health_indicators.append((df['cortisol_level'] - df['cortisol_level'].min()) / 
                                       (df['cortisol_level'].max() - df['cortisol_level'].min()) * 10)
            if 'heart_rate_variability' in df.columns:
                # Invert HRV (lower HRV = higher stress)
                health_indicators.append(10 - ((df['heart_rate_variability'] - df['heart_rate_variability'].min()) / 
                                              (df['heart_rate_variability'].max() - df['heart_rate_variability'].min()) * 10))
            
            if health_indicators:
                df['composite_stress_score'] = np.mean(health_indicators, axis=0)
            else:
                logger.warning("No health indicators found for composite score")
...
class StressPredictionModels:
    ...
    def prepare_features_improved(self) -> Tuple[pd.DataFrame, pd.Series, List[str]]:
        """IMPROVED feature preparation with better selection"""
        try:
            # Select more relevant features
            acoustic_features = [
                'spl_mean', 'spl_std', 'spectral_centroid_mean', 'spectral_bandwidth_mean',
                'roughness', 'sharpness', 'zcr_mean', 'rms_energy', 'loudness',
            ...
            demographic_features = [
                'age', 'cortisol_level', 'heart_rate_variability'
            ]
            
            urban_features = [
                'population_density', 'daily_traffic_count'
            ]
            
            # Combine all feature groups
            all_features = acoustic_features + temporal_features + environmental_features + demographic_features + urban_features
            ...
            if 'gender' in df_encoded.columns:
                gender_dummies = pd.get_dummies(df_encoded['gender'], prefix='gender', drop_first=True)
            ...
            if 'occupation' in df_encoded.columns:
                # Create occupation stress mapping instead of one-hot encoding
                occupation_stress = {
                    'healthcare': 8, 'education': 6, 'finance': 7, 'retail': 5,
                    'technology': 6, 'manufacturing': 5, 'government': 4, 'other': 5
                }
                df_encoded['occupation_stress'] = df_encoded['occupation'].map(occupation_stress).fillna(5)
                available_features.append('occupation_stress')
            
            # Create final feature matrix
            X = df_encoded[available_features].fillna(df_encoded[available_features].median())
            
            # Use composite stress score as target
            y = df_encoded['composite_stress_score']
            
            self.feature_cols = available_features
            logger.info(f"Prepared {len(available_features)} features for improved modeling")
            
            return X, y, available_features
            
        except Exception as e:
            logger.error(f"Error preparing improved features: {e}")
            return pd.DataFrame(), pd.Series(), []
    
    def prepare_features(self) -> Tuple[pd.DataFrame, pd.Series, List[str]]:
        """Prepare features for machine learning - uses improved version"""
        return self.prepare_features_improved()
...
class ComprehensiveAnalyticsPipeline:
    ...
    def __init__(self):
        ...
        self.results = {}
        self.df = pd.DataFrame()  # FIX: Store DataFrame in pipeline
    ...
    def run_complete_analysis(self, data_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # FIX: Store the processed dataframe properly
            self.df = df
    ...
    def _generate_synthetic_data(self, n_samples: int = 1000) -> pd.DataFrame:
        """IMPROVED synthetic data with realistic noise-stress correlations"""
        try:
            ...
            timestamps = [start_date + timedelta(hours=x/24) for x in range(n_samples)]
            
            # Generate base features with time patterns
            hours = [(start_date + timedelta(hours=x/24)).hour for x in range(n_samples)]
            is_rush_hour = [(7 <= h <= 9) or (17 <= h <= 19) for h in hours]
            is_weekend = [((start_date + timedelta(hours=x/24)).weekday() >= 5) for x in range(n_samples)]
            
            # Generate correlated noise levels with time patterns
            base_noise = np.random.normal(60, 8, n_samples)  # Base ~60dB
            
            # Add rush hour effect
            rush_hour_effect = [5 if rush else 0 for rush in is_rush_hour]
            weekend_effect = [-3 if weekend else 0 for weekend in is_weekend]
            
            spl_mean = np.clip(base_noise + np.array(rush_hour_effect) + np.array(weekend_effect) + 
                              np.random.normal(0, 3, n_samples), 35, 90)
            
            # Generate REALISTIC correlated acoustic features
            spectral_centroid_mean = 1500 + spl_mean * 8 + np.random.normal(0, 200, n_samples)
            roughness = np.maximum(0.01, (spl_mean - 45) / 50 * 0.3 + np.random.exponential(0.1, n_samples))
            sharpness = np.maximum(0.1, (spl_mean - 40) / 60 * 2 + np.random.gamma(1.5, 0.3, n_samples))
            zcr_mean = np.clip(0.08 + (spl_mean - 55) / 100 + np.random.normal(0, 0.02, n_samples), 0.01, 0.3)
            rms_energy = np.maximum(0.1, (spl_mean / 70) * 0.6 + np.random.normal(0, 0.1, n_samples))
            loudness = np.maximum(0.05, (spl_mean - 30) / 60 * 0.8 + np.random.normal(0, 0.1, n_samples))
            
            # Generate MFCC features with some correlation to noise
            mfcc_features = {}
            for i in range(config.N_MFCC):
                if i == 0:  # First MFCC often correlates with loudness
                    mfcc_features[f'mfcc_{i}_mean'] = -10 + (spl_mean / 80) * 5 + np.random.normal(0, 2, n_samples)
                else:
                    mfcc_features[f'mfcc_{i}_mean'] = np.random.normal(0, 1.5, n_samples)
                mfcc_features[f'mfcc_{i}_std'] = np.random.exponential(0.8, n_samples)
            
            # Generate STRONGLY CORRELATED health indicators
            # Create individual susceptibility factors
            individual_sensitivity = np.random.normal(1.0, 0.3, n_samples)
            age_factor = np.random.randint(18, 80, n_samples)
            age_stress_factor = np.where(age_factor > 50, 1.2, np.where(age_factor < 30, 1.1, 1.0))
            
            # Calculate noise stress impact (stronger correlation)
            noise_impact = ((spl_mean - 45) / 25) * individual_sensitivity * age_stress_factor
            
            # Generate correlated health outcomes
            sleep_quality = np.clip(8.5 - noise_impact * 1.8 + np.random.normal(0, 0.8, n_samples), 1, 10)
            anxiety_level = np.clip(2 + noise_impact * 2.2 + np.random.normal(0, 0.7, n_samples), 1, 10)
            mood_rating = np.clip(7.5 - noise_impact * 1.5 + np.random.normal(0, 0.8, n_samples), 1, 10)
            depression_score = np.clip(1.5 + noise_impact * 1.3 + np.random.normal(0, 0.6, n_samples), 1, 10)
            
            # Additional health indicators for stronger correlation
            heart_rate_variability = np.clip(50 - noise_impact * 8 + np.random.normal(0, 5, n_samples), 20, 100)
            cortisol_level = np.maximum(5, 8 + noise_impact * 6 + np.random.normal(0, 2, n_samples))
            
            # Demographics with realistic distributions
            ages = age_factor
            genders = np.random.choice(['M', 'F'], n_samples)
            occupations = np.random.choice(['healthcare', 'education', 'finance', 'technology', 'retail', 'other'], 
                                         n_samples, p=[0.15, 0.12, 0.18, 0.20, 0.15, 0.20])
            
            # Environmental factors with seasonal patterns
            day_of_year = [(start_date + timedelta(hours=x/24)).timetuple().tm_yday for x in range(n_samples)]
            seasonal_temp = 15 + 10 * np.sin(2 * np.pi * np.array(day_of_year) / 365)
            temperatures = seasonal_temp + np.random.normal(0, 5, n_samples)
            humidity = np.clip(50 + 20 * np.sin(2 * np.pi * np.array(day_of_year) / 365 + np.pi/4) + 
                              np.random.normal(0, 10, n_samples), 20, 95)
            wind_speed = np.maximum(0, np.random.exponential(8, n_samples))
            precipitation = np.maximum(0, np.random.exponential(1, n_samples))
            
            # Urban context with realistic correlations
            population = np.random.randint(5000, 500000, n_samples)
            area_km2 = np.random.uniform(5, 100, n_samples)
            # Traffic correlates with population and noise
            traffic_base = (population / 10000) * 1000
            daily_traffic_count = np.maximum(100, traffic_base + (spl_mean - 50) * 200 + 
                                           np.random.normal(0, traffic_base * 0.2, n_samples))
            
            # Create DataFrame with all features
            data = {
                'timestamp': timestamps,
                'spl_mean': spl_mean,
                'spl_std': np.random.normal(3, 1, n_samples),
                'spectral_centroid_mean': spectral_centroid_mean,
                'spectral_centroid_std': np.random.normal(150, 30, n_samples),
                'spectral_bandwidth_mean': spectral_centroid_mean * 0.6 + np.random.normal(0, 100, n_samples),
                'spectral_bandwidth_std': np.random.normal(80, 20, n_samples),
                'spectral_rolloff_mean': spectral_centroid_mean * 1.8 + np.random.normal(0, 200, n_samples),
                'spectral_rolloff_std': np.random.normal(200, 50, n_samples),
                'roughness': roughness,
                'sharpness': sharpness,
                'zcr_mean': zcr_mean,
                'zcr_std': np.random.normal(0.015, 0.005, n_samples),
                'rms_energy': rms_energy,
                'chroma_mean': np.random.uniform(0.3, 0.7, n_samples),
                'chroma_std': np.random.uniform(0.1, 0.3, n_samples),
                'loudness': loudness,
                'onset_rate': np.random.exponential(2, n_samples),
                'tempo': np.random.uniform(60, 140, n_samples),
            ...
            logger.info("Improved synthetic data generated successfully")
            return df
...
def test_fixes():
    """Test the fixes with improved correlation"""
    try:
        print("Testing fixes...")
        
        # Create pipeline with fixes
        pipeline = ComprehensiveAnalyticsPipeline()
        
        # Generate better synthetic data
        df = pipeline._generate_synthetic_data(500)  # Smaller sample for testing
        
        # Check correlations
        if 'spl_mean' in df.columns and 'composite_stress_score' in df.columns:
            correlation = df['spl_mean'].corr(df['composite_stress_score'])
            print(f"Noise-Stress Correlation: {correlation:.4f}")
            
            if abs(correlation) > 0.3:
                print("✅ Strong correlation achieved!")
            else:
                print("⚠️ Correlation could be stronger")
        
        print(f"Dataset shape: {df.shape}")
        print(f"Available columns: {list(df.columns)}")
        
        # Test composite stress score creation
        pipeline.health_processor.process_survey_data(df)
        if 'composite_stress_score' in df.columns:
            print(f"✅ Composite stress score created successfully")
            print(f"Stress score range: {df['composite_stress_score'].min():.2f} - {df['composite_stress_score'].max():.2f}")
...
def main():
    ...
    try:
        ...
        if results:
            ...
            # FIX: Get the processed dataframe properly
            processed_df = pipeline.df if hasattr(pipeline, 'df') and not pipeline.df.empty else pd.DataFrame()
            
            if not processed_df.empty:
                # Create interactive dashboard
                dashboard = InteractiveDashboard(processed_df, results)
                figures = dashboard.create_plotly_dashboard()
                dashboard.save_dashboard_html(figures)
            else:
                logger.warning("No processed dataframe available for dashboard")
...
from fixed_noise_mental_health_analytics import ComprehensiveAnalyticsPipeline

pipeline = ComprehensiveAnalyticsPipeline()
results = pipeline.run_complete_analysis("path/to/your/data.csv")

# Test the fixes
from fixed_noise_mental_health_analytics import test_fixes
test_fixes()

# Real-time monitoring
from fixed_noise_mental_health_analytics import RealTimeStressMonitor, SoundscapeAnalyzer
import joblib
...
# Policy analysis
from fixed_noise_mental_health_analytics import PolicyImpactSimulator
import pandas as pd
...
# WHAT'S FIXED:
# 1. Dashboard error - properly stores and retrieves processed DataFrame
# 2. Improved synthetic data generation with stronger noise-stress correlations
# 3. Better composite stress score calculation using multiple health indicators
# 4. Enhanced feature selection for machine learning models
# 5. Added test function to verify fixes work properly
""" 